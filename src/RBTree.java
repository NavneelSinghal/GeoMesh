public class RBTree<T extends Comparable<T>, E extends Comparable<E>>
    implements RBTreeInterface<T, E> {
        ...
        if (r != null) {
            ...
            while (node != null && node != root && node.colour == 0) {
                if (node.parent.colour == 1) {
                    ...
                    // parent of the node is red now, so the grandparent always
                    // exists. when you rotate, just check if the node you
                    // rotate is null or not.
                    ...
                    if (node.parent == node.parent.parent.left) {
                        ...
                        if (node == node.parent.left) {
                            ...
                            node =
                                node.right; // reduction to the right line case
                        }
